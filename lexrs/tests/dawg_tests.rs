...
fn test_batch_contains() {
    let mut dawg = Dawg::new();
    dawg.add_all(["apple", "apply", "apt", "banana"].map(String::from)).unwrap();
    assert_eq!(
        dawg.batch_contains(&["apple", "cherry", "apt"]),
        vec![true, false, true]
    );
}
...
fn test_batch_contains_empty() {
    let dawg = Dawg::new();
    let result: Vec<bool> = dawg.batch_contains(&[] as &[&str]);
...
fn test_batch_search() {
    let mut dawg = Dawg::new();
    dawg.add_all(["apple", "apply", "apt", "banana"].map(String::from)).unwrap();
    let result = dawg.batch_search(&["ap*", "b*"]).unwrap();
...
fn test_batch_search_with_prefix() {
    let mut dawg = Dawg::new();
    dawg.add_all(["apple", "apply", "apt", "banana"].map(String::from)).unwrap();
    let result = dawg.batch_search_with_prefix(&["app", "ban", "xyz"]);
...
fn test_batch_search_within_distance() {
    let mut dawg = Dawg::new();
    dawg.add_all(["apple", "apply", "apt", "banana"].map(String::from)).unwrap();
    let result = dawg.batch_search_within_distance(&["aple", "bananaa"], 1);
...
fn test_batch_preserves_order() {
    let mut dawg = Dawg::new();
    dawg.add_all(["apple", "banana"].map(String::from)).unwrap();
    let result = dawg.batch_contains(&["banana", "cherry", "apple"]);
